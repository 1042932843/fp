...
public class StatisticsDetailTypeFragment extends BaseFragment implements OnChartValueSelectedListener {
    ArrayList<PoertyType> types;
    ...
    public static StatisticsDetailTypeFragment newInstance() {
        StatisticsDetailTypeFragment fragment=  new StatisticsDetailTypeFragment();
        return fragment;
    }
    @Override
    public int getLayoutResId() {
        return R.layout.fragment_statistics_type;
    }
    ...
    public void onEvent(String event){
        if(!TextUtils.isEmpty(event)) {
            switch (event) {
                case "StatisticsDetailTypeFragment":
                    loadData();
                    break;
    ...
    private void setData() {
        ...
        PieDataSet dataSet = new PieDataSet(entries, "贫困类型分布");
    ...
    public void loadData(){
        ...
        RetrofitHelper.getStatisticsAPI()
                .getpovertyType(StatisticsDetailActivity.permissionstag)
                    ...
                    switch (state){
                        case "0":
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                PoertyType type=JSON.parseObject(jsonArray.get(i).toString(),PoertyType.class);
                                if(type!=null){
                                    types.add(type);
                                    ...
                                    entries.add(new PieEntry(Float.parseFloat(type.getNum()),type.getPovertyattribute()+"("+type.getNum()+")"));
                                }
                            }
